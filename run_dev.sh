TMP_ENV=$(mktemp)
cleanup() {
  echo "Cleaning up..."
  rm -f "$TMP_ENV"
}
trap cleanup EXIT

echo "Fetching secrets..."
SECRET_JSON=$(gcloud secrets versions access latest --secret=nmwdiproduction_dagster_psql)
PG_USER=$(echo "$SECRET_JSON" | jq -r '.user')
PG_PASSWORD=$(echo "$SECRET_JSON" | jq -r '.password')

(
  printf "PG_USER=$PG_USER\n"
  printf "PG_PASSWORD=$PG_PASSWORD\n"
) > $TMP_ENV

docker compose -p dagster down --remove-orphans &&
docker compose -p dagster -f ./compose.common.yaml -f ./compose.dev.yaml --env-file "$TMP_ENV" up --build --force-recreate
